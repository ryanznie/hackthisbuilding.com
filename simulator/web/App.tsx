...
async function api<T>(path: string, body?: object): Promise<T> {
  ...
  try {
    response = await fetch(path, { method: body ? 'POST' : 'GET', headers: body ? { 'Content-Type': 'application/json' } : {}, body: body ? JSON.stringify(body) : undefined, credentials: 'same-origin', signal: AbortSignal.timeout(path === '/api/preview' ? 60000 : 15000) });
  } catch {
