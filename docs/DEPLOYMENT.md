...
The Worker exposes the current rendered 17-row × 9-column frame at authenticated `GET /api/display/frame`. The runner validates every RGB channel, converts the frame to the documented 459-byte row-major format, and sends through a `makeframe()`/`send()` adapter. Static clips are sent once. Dynamic clips and the invitation URL are sampled at no more than 30 FPS, below the simulator's 40-request-per-second ceiling. Each send waits for the target’s HTTP acknowledgment before deduplication; failed static sends are retried. Five consecutive source or target failures stop the runner. Slow networks reduce the sampling rate. Intrinsically animated shapes remain dynamic even when their layer transform is still.
