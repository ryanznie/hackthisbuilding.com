...
The bridge source is in [`display_runner/`](../display_runner/README.md). Only start it with the organizer-issued instance assigned to this project. The actual instance is not established in this acceptance record; a teammate's example instance is not authorization to claim it. The bridge targets the organizer simulator API and does not itself establish a physical MIT-building connection.
